...
df_dc = read.csv('E:/2022/Freelance/Jobs/001-25/data-dc.csv', header=T)
df_dc = data.frame(time=df_dc$Time..hours., fails=df_dc$Number.of.failures)

time_rep = rep(df_dc$time, df_dc$fails)
descdist(time_rep)
...
fit_beta_mge.2 = fitdist(time_rep_beta, 'beta', method='mge', gof='CvM') # Doesn't work
...
## From the info_df data we can say that the weibull distribution with the MLE
## method conclude the best results for the dccc data
